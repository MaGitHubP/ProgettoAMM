<?php
    if(isset($wrongLogin) && $wrongLogin!=0){
	if($wrongLogin==1){
	    $wrongLogin=0;
	}else if($wrongLogin==2){
	    $wrongLogin=0;
	    echo "Errore:Un venditore non può comprare.\n";
	}
    }

    if(isset($notLogged) && $notLogged==true){
        $notLogged=false;   ?>
        <p id="main_p">Per acquistare qualcosa, devi prima accedere:</p>
        <br/>
		...
		if(isset($research_name)){
			...
			<p id="game_price"><b>Prezzo:</b> <?= $row[5] ?></p>
                        <form method="post" action="purchase">
                            <input type="hidden" name="cmd" value="loginToBuy"/>
    ...
    <?php	if(isset($research_name)){
		    	...
		    	<a href="login?cmd=search&research_name=true&namesearch=<?= $name_game ?>&limit_index=<?= $limit_back ?>" id="prev">Indietro</a>
    <?php           }
		    $offset=$limit_index+4;
		    if(($offset) < $total_result){
		        $limit_front=$offset;	?>
                        <a href="login?cmd=search&research_name=true&namesearch=<?= $name_game ?>&limit_index=<?= $limit_front ?>" id="next">Avanti</a>
    <?php           }
    		}else{
	            if($limit_index!=0){	
		        $limit_back=$limit_index-4;	?>
		        <a href="login?cmd=search&genre=<?= $genre ?>&console=<?= $console ?>&limit_index=<?= $limit_back ?>" id="prev">Indietro</a>
    <?php           }
		    $offset=$limit_index+4;
		    if(($offset) < $total_result){
		        $limit_front=$offset;	?>
                        <a href="login?cmd=search&genre=<?= $genre ?>&console=<?= $console ?>&limit_index=<?= $limit_front ?>" id="next">Avanti</a>
    <?php           }
    		}
