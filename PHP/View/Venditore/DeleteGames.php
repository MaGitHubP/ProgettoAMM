<p id="main_p">Scegli qui sotto il videogioco che vuoi eliminare.</p>
...
<?php
	...
	<?php	echo "<ul>\n";
		while($row=$result->fetch_row()){
		    ...
		    <a href="seller?cmd=delete&videogame_id=<?= $row[0] ?>" class="deletegame">Elimina</a>
	<?php	    echo "</li>";
		}
		echo "</n>\n";
...
<?php       if($limit_index!=0){	
		...
		<a href="seller?subpage=delete&limit_index=<?= $limit_back ?>" id="prev">Indietro</a>
<?php       }
	    $offset=$limit_index+4;
	    if(($offset) < $total_result){
		$limit_front=$offset;	?>
                <a href="seller?subpage=delete&limit_index=<?= $limit_front ?>" id="next">Avanti</a>
<?php       }
        }?>
